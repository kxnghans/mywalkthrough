                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {awards.map((award, index) => (
                        <div key={index} 
                             ...
                             onClick={() => setSelectedItem(award.details)}>
                            <img src={award.imageUrl} alt={award.title} className="w-full h-40 object-cover" />
                            <div className="p-4">
                                <h3 className="text-lg font-bold mb-1 text-gray-900 dark:text-gray-300 truncate">{award.title}</h3>
                                <div className="text-gray-600 dark:text-gray-400 text-sm h-10 overflow-hidden">
                                    {award.summary.map((line, i) => (
